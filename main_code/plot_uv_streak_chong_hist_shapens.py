...
# -----------------------------------------------------------------------------------------------------------------------
xlabel            = "$\Omega/\Omega_{tot}$"
ylabel            = r"$\phi^{\mathbf{u}}$"
xlabelvol         = r"$\Omega/(\Omega_{tot}V^+)$"
ylabelvol         = r"$\phi^{\mathbf{u}}/V^+$"
zlabel            = "$ $"
fontsize          = 18
figsize_x         = 8
figsize_y         = 7
...
# Define functions
# -----------------------------------------------------------------------------------------------------------------------
def save_frequency(data_in={'file_frequency':'frequency.h5','grid_shapQ2':[],'grid_ensQ2':[],'grid_shapens_totQ2':[],
                            'grid_shapQ4':[],'grid_ensQ4':[],'grid_shapens_totQ4':[],'grid_shap_streakh':[],
                            'grid_ens_streakh':[],'grid_shapens_tot_streakh':[],'grid_shap_streakl':[],
                            'grid_ens_streakl':[],'grid_shapens_tot_streakl':[],'grid_shap_chong':[],
                            'grid_ens_chong':[],'grid_shapens_tot_chong':[],'last_field':0,'n_fields':0,
                            "ens_min":0,"ens_max":1,"shap_min":0,"shap_max":1}):
    file_frequency           = str(data_in['file_frequency'])
    grid_shapQ2              = np.array(data_in['grid_shapQ2'])
    grid_ensQ2               = np.array(data_in['grid_ensQ2'])
    grid_shapens_totQ2       = np.array(data_in['grid_shapens_totQ2'])
    grid_shapQ4              = np.array(data_in['grid_shapQ4'])
    grid_ensQ4               = np.array(data_in['grid_ensQ4'])
    grid_shapens_totQ4       = np.array(data_in['grid_shapens_totQ4'])
    grid_shap_streakh        = np.array(data_in['grid_shap_streakh'])
    grid_ens_streakh         = np.array(data_in['grid_ens_streakh'])
    grid_shapens_tot_streakh = np.array(data_in['grid_shapens_tot_streakh'])
    grid_shap_streakl        = np.array(data_in['grid_shap_streakl'])
    grid_ens_streakl         = np.array(data_in['grid_ens_streakl'])
    grid_shapens_tot_streakl = np.array(data_in['grid_shapens_tot_streakl'])
    grid_shap_chong          = np.array(data_in['grid_shap_chong'])
    grid_ens_chong           = np.array(data_in['grid_ens_chong'])
    grid_shapens_tot_chong   = np.array(data_in['grid_shapens_tot_chong'])
    last_field               = np.array(data_in['last_field'])
    n_fields                 = np.array(data_in['n_fields'])
    ens_min                  = float(data_in['ens_min'])
    ens_max                  = float(data_in['ens_max'])
    ...
    ff_freq.create_dataset('grid_ensQ2',data=grid_ensQ2)
    ff_freq.create_dataset('grid_shapens_totQ2',data=grid_shapens_totQ2)
    ff_freq.create_dataset('grid_shapQ4',data=grid_shapQ4)
    ff_freq.create_dataset('grid_ensQ4',data=grid_ensQ4)
    ff_freq.create_dataset('grid_shapens_totQ4',data=grid_shapens_totQ4)
    ff_freq.create_dataset('grid_shap_streakh',data=grid_shap_streakh)
    ff_freq.create_dataset('grid_ens_streakh',data=grid_ens_streakh)
    ff_freq.create_dataset('grid_shapens_tot_streakh',data=grid_shapens_tot_streakh)
    ff_freq.create_dataset('grid_shap_streakl',data=grid_shap_streakl)
    ff_freq.create_dataset('grid_ens_streakl',data=grid_ens_streakl)
    ff_freq.create_dataset('grid_shapens_tot_streakl',data=grid_shapens_tot_streakl)
    ff_freq.create_dataset('grid_shap_chong',data=grid_shap_chong)
    ff_freq.create_dataset('grid_ens_chong',data=grid_ens_chong)
    ff_freq.create_dataset('grid_shapens_tot_chong',data=grid_shapens_tot_chong)
    ff_freq.create_dataset('last_field',data=last_field)
    ff_freq.create_dataset('n_fields',data=n_fields)
    ff_freq.create_dataset('ens_min',data=ens_min)
    ff_freq.create_dataset('ens_max',data=ens_max)
...
ens_folder        = folders.ens_folder
ens_file          = folders.ens_file
...
data_shap_uv    = {"shap_folder":shapfolder_uv,"shap_file":shapfile_uv,"uvw_folder":uvw_folder,"uvw_file":uvw_file,
                   ...
                   "nfil":nfil,"stride":stride,"activation":activation,"kernel":kernel,"pooling":pooling,
                   "delta_pred":1,"nsamples":0,"nsamples_max":0,"data_type":"float32",
...
for index in range(field_ini,field_fin,field_delta):
    ...
    uv_struc.calc_enstrophy(data_in={"ens_folder":ens_folder,"ens_file":ens_file})
    ...
    streak_struc.calc_enstrophy(data_in={"ens_folder":ens_folder,"ens_file":ens_file})
    ...
    chong_struc.calc_enstrophy(data_in={"ens_folder":ens_folder,"ens_file":ens_file})
    ...
    ensvol_max_streak  = np.max([np.max(ensvol_streakh),np.max(ensvol_streakl)])
    ensvol_min_streak  = np.min([np.min(ensvol_streakh),np.min(ensvol_streakl)])
    
    
    vol_chong         = chong_struc.structures.vol[shap_ind_chong]
    ens_chong         = chong_struc.ens_struc[shap_ind_chong]
    shapvol_chong     = shap_chong/vol_chong
    ensvol_chong      = ens_chong/vol_chong
    shap_max_chong    = np.max(shap_chong)
    shap_min_chong    = np.min(shap_chong)
    ens_max_chong     = np.max(ens_chong) 
    ens_min_chong     = np.min(ens_chong)
    shapvol_max_chong = np.max(shapvol_chong)
    shapvol_min_chong = np.min(shapvol_chong)
    ensvol_max_chong  = np.max(ensvol_chong)
    ensvol_min_chong  = np.min(ensvol_chong)
    ...
    ens_min     = np.min([ens_min,ens_min_uv,ens_min_streak,ens_min_chong])
    ens_max     = np.max([ens_max,ens_max_uv,ens_max_streak,ens_max_chong])
    ...
    ensvol_min  = np.min([ensvol_min,ensvol_min_uv,ensvol_min_streak,ensvol_min_chong])
    ensvol_max  = np.max([ensvol_max,ensvol_max_uv,ensvol_max_streak,ensvol_max_chong])
    
    
    bins_shap    = np.linspace(shap_min,shap_max,bins)
    bins_shapvol = np.linspace(shapvol_min,shapvol_max,bins)
    bins_ens     = np.linspace(ens_min,ens_max,bins)
    bins_ensvol  = np.linspace(ensvol_min,ensvol_max,bins)
    
    # ----------------------------------------------------------------------------------------------------------------
    # Calculate the pdf for shap and volume
    # ----------------------------------------------------------------------------------------------------------------
    hist_shapensQ2,hist_ensQ2,hist_shapQ2 = np.histogram2d(ensQ2,shapQ2,bins=(bins_ens,bins_shap))
    hist_ensQ2                            = hist_ensQ2[:-1]+np.diff(hist_ensQ2)/2
    hist_shapQ2                           = hist_shapQ2[:-1]+np.diff(hist_shapQ2)/2
    grid_ensQ2,grid_shapQ2                = np.meshgrid(hist_ensQ2,hist_shapQ2)
    grid_shapensQ2                        = hist_shapensQ2.T.copy()
    if index == field_ini:
        grid_shapens_totQ2  = grid_shapensQ2
    else:
        grid_shapens_totQ2 += grid_shapensQ2
    hist_shapensQ4,hist_ensQ4,hist_shapQ4 = np.histogram2d(ensQ4,shapQ4,bins=(bins_ens,bins_shap))
    hist_ensQ4                            = hist_ensQ4[:-1]+np.diff(hist_ensQ4)/2
    hist_shapQ4                           = hist_shapQ4[:-1]+np.diff(hist_shapQ4)/2
    grid_ensQ4,grid_shapQ4                = np.meshgrid(hist_ensQ4,hist_shapQ4)
    grid_shapensQ4                        = hist_shapensQ4.T.copy()
    if index == field_ini:
        grid_shapens_totQ4  = grid_shapensQ4
    else:
        grid_shapens_totQ4 += grid_shapensQ4
    hist_shapens_streakh,hist_ens_streakh,hist_shap_streakh = np.histogram2d(ens_streakh,shap_streakh,
                                                                             bins=(bins_ens,bins_shap))
    hist_ens_streakh                                        = hist_ens_streakh[:-1]+np.diff(hist_ens_streakh)/2
    hist_shap_streakh                                       = hist_shap_streakh[:-1]+np.diff(hist_shap_streakh)/2
    grid_ens_streakh,grid_shap_streakh                      = np.meshgrid(hist_ens_streakh,hist_shap_streakh)
    grid_shapens_streakh                                    = hist_shapens_streakh.T.copy()
    if index == field_ini:
        grid_shapens_tot_streakh  = grid_shapens_streakh
    else:
        grid_shapens_tot_streakh += grid_shapens_streakh
    hist_shapens_streakl,hist_ens_streakl,hist_shap_streakl = np.histogram2d(ens_streakl,shap_streakl,
                                                                             bins=(bins_ens,bins_shap))
    hist_ens_streakl                                        = hist_ens_streakl[:-1]+np.diff(hist_ens_streakl)/2
    hist_shap_streakl                                       = hist_shap_streakl[:-1]+np.diff(hist_shap_streakl)/2
    grid_ens_streakl,grid_shap_streakl                      = np.meshgrid(hist_ens_streakl,hist_shap_streakl)
    grid_shapens_streakl                                    = hist_shapens_streakl.T.copy()
    if index == field_ini:
        grid_shapens_tot_streakl  = grid_shapens_streakl
    else:
        grid_shapens_tot_streakl += grid_shapens_streakl
        
        
    hist_shapens_chong,hist_ens_chong,hist_shap_chong = np.histogram2d(ens_chong,shap_chong,
                                                                       bins=(bins_ens,bins_shap))
    hist_ens_chong                                    = hist_ens_chong[:-1]+np.diff(hist_ens_chong)/2
    hist_shap_chong                                   = hist_shap_chong[:-1]+np.diff(hist_shap_chong)/2
    grid_ens_chong,grid_shap_chong                    = np.meshgrid(hist_ens_chong,hist_shap_chong)
    grid_shapens_chong                                = hist_shapens_chong.T.copy()
    if index == field_ini:
        grid_shapens_tot_chong  = grid_shapens_chong
    else:
        grid_shapens_tot_chong += grid_shapens_chong
    ...
    # Calculate the pdf for shap and volume
    # ----------------------------------------------------------------------------------------------------------------
    hist_shapvol_ensvolQ2,hist_ensvolQ2,hist_shapvolQ2 = np.histogram2d(ensvol_Q2,shapvol_Q2,
                                                                        bins=(bins_ensvol,bins_shapvol))
    hist_ensvolQ2                                      = hist_ensvolQ2[:-1]+np.diff(hist_ensvolQ2)/2
    hist_shapvolQ2                                     = hist_shapvolQ2[:-1]+np.diff(hist_shapvolQ2)/2
    grid_ensvolQ2,grid_shapvolQ2                       = np.meshgrid(hist_ensvolQ2,hist_shapvolQ2)
    grid_shapvol_ensvolQ2                              = hist_shapvol_ensvolQ2.T.copy()
    if index == field_ini:
        grid_shapvol_ensvol_totQ2  = grid_shapvol_ensvolQ2
    else:
        grid_shapvol_ensvol_totQ2 += grid_shapvol_ensvolQ2
    hist_shapvol_ensvolQ4,hist_ensvolQ4,hist_shapvolQ4 = np.histogram2d(ensvol_Q4,shapvol_Q4,
                                                                        bins=(bins_ensvol,bins_shapvol))
    hist_ensvolQ4                                      = hist_ensvolQ4[:-1]+np.diff(hist_ensvolQ4)/2
    hist_shapvolQ4                                     = hist_shapvolQ4[:-1]+np.diff(hist_shapvolQ4)/2
    grid_ensvolQ4,grid_shapvolQ4                       = np.meshgrid(hist_ensvolQ4,hist_shapvolQ4)
    grid_shapvol_ensvolQ4                              = hist_shapvol_ensvolQ4.T.copy()
    if index == field_ini:
        grid_shapvol_ensvol_totQ4  = grid_shapvol_ensvolQ4
    else:
        grid_shapvol_ensvol_totQ4 += grid_shapvol_ensvolQ4
        
    hist_shapvol_ensvol_streakh,hist_ensvol_streakh,hist_shapvol_streakh = np.histogram2d(ensvol_streakh,
                                                                                          shapvol_streakh,
                                                                                          bins=(bins_ensvol,
                                                                                                bins_shapvol))
    hist_ensvol_streakh                                 = hist_ensvol_streakh[:-1]+np.diff(hist_ensvol_streakh)/2
    hist_shapvol_streakh                                = hist_shapvol_streakh[:-1]+np.diff(hist_shapvol_streakh)/2
    grid_ensvol_streakh,grid_shapvol_streakh            = np.meshgrid(hist_ensvol_streakh,hist_shapvol_streakh)
    grid_shapvol_ensvol_streakh                         = hist_shapvol_ensvol_streakh.T.copy()
    if index == field_ini:
        grid_shapvol_ensvol_tot_streakh  = grid_shapvol_ensvol_streakh
    else:
        grid_shapvol_ensvol_tot_streakh += grid_shapvol_ensvol_streakh
        
    hist_shapvol_ensvol_streakl,hist_ensvol_streakl,hist_shapvol_streakl = np.histogram2d(ensvol_streakl,
                                                                                          shapvol_streakl,
                                                                                          bins=(bins_ensvol,
                                                                                                bins_shapvol))
    hist_ensvol_streakl                                 = hist_ensvol_streakl[:-1]+np.diff(hist_ensvol_streakl)/2
    hist_shapvol_streakl                                = hist_shapvol_streakl[:-1]+np.diff(hist_shapvol_streakl)/2
    grid_ensvol_streakl,grid_shapvol_streakl            = np.meshgrid(hist_ensvol_streakl,hist_shapvol_streakl)
    grid_shapvol_ensvol_streakl                         = hist_shapvol_ensvol_streakl.T.copy()
    if index == field_ini:
        grid_shapvol_ensvol_tot_streakl  = grid_shapvol_ensvol_streakl
    else:
        grid_shapvol_ensvol_tot_streakl += grid_shapvol_ensvol_streakl
        
        
    hist_shapvol_ensvol_chong,hist_ensvol_chong,hist_shapvol_chong = np.histogram2d(ensvol_chong,shapvol_chong,
                                                                                    bins=(bins_ensvol,bins_shapvol))
    hist_ensvol_chong                                        = hist_ensvol_chong[:-1]+np.diff(hist_ensvol_chong)/2
    hist_shapvol_chong                                       = hist_shapvol_chong[:-1]+np.diff(hist_shapvol_chong)/2
    grid_ensvol_chong,grid_shapvol_chong                     = np.meshgrid(hist_ensvol_chong,hist_shapvol_chong)
    grid_shapvol_ensvol_chong                                = hist_shapvol_ensvol_chong.T.copy()
    if index == field_ini:
        grid_shapvol_ensvol_tot_chong  = grid_shapvol_ensvol_chong
    else:
        grid_shapvol_ensvol_tot_chong += grid_shapvol_ensvol_chong
    # ----------------------------------------------------------------------------------------------------------------
    # Calculate the pdf for shap and volume
    ...
    try:
        save_frequency(data_in={'file_frequency':folder_save+'/'+file_frequency,'grid_shapQ2':grid_shapQ2,
                                'grid_ensQ2':grid_ensQ2,'grid_shapens_totQ2':grid_shapens_totQ2,
                                'grid_shapQ4':grid_shapQ4,'grid_ensQ4':grid_ensQ4,
                                'grid_shapens_totQ4':grid_shapens_totQ4,'grid_shap_streakh':grid_shap_streakh,
                                'grid_ens_streakh':grid_ens_streakh,'grid_shapens_tot_streakh':grid_shapens_tot_streakh,
                                'grid_shap_streakl':grid_shap_streakl,'grid_ens_streakl':grid_ens_streakl,
                                'grid_shapens_tot_streakl':grid_shapens_tot_streakl,'grid_shap_chong':grid_shap_chong,
                                'grid_ens_chong':grid_ens_chong,'grid_shapens_tot_chong':grid_shapens_tot_chong,
                                'last_field':last_field,'n_fields':n_fields,"ens_min":ens_min,"ens_max":ens_max,
        ...
        save_frequency(data_in={'file_frequency':folder_save+'/'+file_frequencyvol,'grid_shapQ2':grid_shapvolQ2,
                                'grid_ensQ2':grid_ensvolQ2,'grid_shapens_totQ2':grid_shapvol_ensvol_totQ2,
                                'grid_shapQ4':grid_shapvolQ4,'grid_ensQ4':grid_ensvolQ4,
                                'grid_shapens_totQ4':grid_shapvol_ensvol_totQ4,'grid_shap_streakh':grid_shapvol_streakh,
                                'grid_ens_streakh':grid_ensvol_streakh,
                                'grid_shapens_tot_streakh':grid_shapvol_ensvol_tot_streakh,
                                'grid_shap_streakl':grid_shapvol_streakl,'grid_ens_streakl':grid_ensvol_streakl,
                                'grid_shapens_tot_streakl':grid_shapvol_ensvol_tot_streakl,
                                'grid_shap_chong':grid_shapvol_chong,'grid_ens_chong':grid_ensvol_chong,
                                'grid_shapens_tot_chong':grid_shapvol_ensvol_tot_chong,
                                'last_field':last_field,'n_fields':n_fields,"ens_min":ensvol_min,"ens_max":ensvol_max,
